...
public class DeviceVoiceAdapter_back extends BaseAdapter {
    ...
    public DeviceVoiceAdapter_back(Context ctx, List<DeviceVoiceEntity> data, String device_id) {
        context = ctx;
                    ...
                    if (temp.exists()) {
                        ...
                        player.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                            ...
                            public void onCompletion(MediaPlayer players) {
                                ...
                                //TODO 回调播放下一首未读的非自己的语音
//                                if(position<dataList.size()-1 && //判断是否到了数据的末尾
//                                        !dataList.get(position+1).getUserid().
//                                                equals(BaseApplication.getInstances().getUser_id()) && //不是自己的语音
//                                        !dataList.get(position+1).isReadMessage() //是未读消息
//                                        ){
//                                    playVoice(BABY, (ImageView)(((View)getItem(position+1)).findViewById(R.id.baby_voice_volumn)), dataList.get(position+1).getVoice_path(),position+1);
//                                    //TODO 重置未读的标记，变更数据中数据，改变未读标记View的显示状态
//                                    dataList.get(position+1).setReadFlag();
//                                    DeviceVoiceHandler.updataValue(dataList.get(position+1));
//                                    (((View)getItem(position+1)).findViewById(R.id.baby_unread_flag)).setVisibility(View.GONE);
////                                    notifyDataSetChanged();//TODO 刷新界面
//                                }
                                if(dataList.get(position).isReadMessage()){//TODO 已读语音不进行连播
                                    return;
                                }
                                for(int i = position+1;i<dataList.size() &&
                                        !dataList.get(i).getUserid().
                                                equals(BaseApplication.getInstances().getUser_id()) &&
                                        !dataList.get(i).isReadMessage();i++){
                                    playVoice(BABY, (ImageView)(((View)getItem(i)).findViewById(R.id.baby_voice_volumn)), dataList.get(i).getVoice_path(),i,true);
                                    //TODO 重置未读的标记，变更数据中数据，改变未读标记View的显示状态
