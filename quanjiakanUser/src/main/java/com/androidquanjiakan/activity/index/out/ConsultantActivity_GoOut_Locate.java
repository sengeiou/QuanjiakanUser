...
public class ConsultantActivity_GoOut_Locate extends BaseActivity implements OnClickListener,LocationSource,
    ...
    private RelativeLayout relocate;
    ...
    public void filter(){
        LogUtil.e("省："+condition_left.getText().toString()+
        "市:"+condition_middle.getText().toString()+
        "区:"+condition_right.getText().toString());
...
//                                    select_condition_middleright.setText(area.get(0));
            ...
            case CITY://市
                if ("全部".equals(condition_left.getText().toString()) || !arrProvinces.contains(condition_left.getText().toString())) {
                    BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,"请选择一个确定的省份!");
                    hideCondition();
                } else if (data == null) {
                    BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,"该地区无更小行政区域");
                    hideCondition();
                } else {
...
//                                        condition_right.setText(area.get(0));
            ...
            case AREA://区
                if ("全部".equals(condition_middle.getText().toString())) {
                    BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,"请选择一个确定的城市!");
                    hideCondition();
                } else if (data == null) {
                    BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,"该地区无更小行政区域");
                    hideCondition();
    ...
    public void initTitleBar(){
        ...
        relocate = (RelativeLayout) findViewById(R.id.relocate);
//        relocate.setOnClickListener(this);
        relocate.setOnTouchListener(new View.OnTouchListener() {
            @Override
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.condition_line_left:
                if (currentTypeValue == PROVINCE && isConditionShow()) {
                    hideCondition();
                } else {
                    showSelectCondition(PROVINCE, arrProvinces);
                }
                currentTypeValue = PROVINCE;
                break;
            case R.id.condition_line_middle:
                if (currentTypeValue == CITY && isConditionShow()) {
                    hideCondition();
                } else {
                    ArrayList<String> city = mCitisDatasMap.get(condition_left.getText().toString());
                    showSelectCondition(CITY, city);
                }
                currentTypeValue = CITY;
                break;
            case R.id.condition_line_right:
                if (currentTypeValue == AREA && isConditionShow()) {
                    hideCondition();
                } else {
                    ArrayList<String> area = mAreaDatasMap.get(condition_middle.getText().toString());
                    showSelectCondition(AREA, area);
                }
                currentTypeValue = AREA;
                break;
...
//                LogUtil.e("");
                ...
                LogUtil.e("Qprovince:"+condition_left.getText().toString()+
                        "Qcity:"+condition_middle.getText().toString()+
                        "Qdistrict:"+condition_right.getText().toString()
                );
                ...
                BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,errText);
            }
        }
    }
    ...
    public void getLatLng(final String address,final String city) {
        LogUtil.e("getLatLng---address:"+address+"\ncity:"+city);
    ...
    public void onRegeocodeSearched(RegeocodeResult result, int rCode) {
        if (rCode == 1000) {
            if (result != null && result.getRegeocodeAddress() != null
                ...
                BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,"address:"+address
                ...
                LogUtil.e("Rprovince:"+condition_left.getText().toString()+
                        "Rcity:"+condition_middle.getText().toString()+
                        "Rdistrict:"+condition_right.getText().toString()
                );

                addMarkerAndInfoWindow();
            } else {
                BaseApplication.getInstances().toast(ConsultantActivity_GoOut_Locate.this,"对不起，没有搜索到相关数据！");
            }
        } else {
            BaseApplication.getInstances().showerror(ConsultantActivity_GoOut_Locate.this,rCode);
        }
    }
    ...
    public void onGeocodeSearched(GeocodeResult result, int rCode) {
        if (rCode == 1000) {
            if (result != null && result.getGeocodeAddressList() != null
                ...
                LogUtil.e("地理编码："+latitude+"---"+longitude);
                addMarkerAndInfoWindow();

            } else {
                ToastUtil.show(ConsultantActivity_GoOut_Locate.this,"对不起，没有搜索到相关数据！");
            }

        } else {
            ToastUtil.showerror(ConsultantActivity_GoOut_Locate.this, rCode);
        }
    }
    ...
    double mylatitude = -181;
    double mylongitude = -181;
    ...
    public View getInfoWindow(Marker marker) {
        ...
        if(address.length()>30){
            String temp  = address.substring(0,15)+"\n"+address.substring(15,30)+"\n"+address.substring(30);
            address = temp;
