...
public class CropImageActivity extends MonitoredActivity {
    ...
    private void setupFromIntent() {
        ...
        File files = new File(getAbsoluteImagePath(sourceUri));
        if(sourceUri != null) {
            ...
            try {
                ...
                if(is==null && files!=null && files.exists()){
                    is = new FileInputStream(files);
                }else if(is==null){
                    is = new FileInputStream(new File(sourceUri.getEncodedPath()));
                }
                ...
                if(outputStream != null) {
                    ...
                    if(exifRotation > 0) {
                        try {
                            ...
                            LogUtil.e("Crop:width---"+width+"   height---"+height);
                            Bitmap bMapRotate =
                                Bitmap.createBitmap( croppedImage, 0, 0, width, height, matrix,
                                    true);

                            bMapRotate.compress( Bitmap.CompressFormat.JPEG, 70, outputStream);
...
//                        int scale = 8000/croppedImage.getWidth();
//                        LogUtil.e("Crop:scale---"+scale);
                        croppedImage.compress( Bitmap.CompressFormat.JPEG, 99, outputStream);
                    }
                }
            } catch(IOException e) {
                setResultException( e);
...
//            }
    ...
    public static Bitmap getSmallBitmap(String filePath) {
        ...
        options.inSampleSize = ImageUtils.calculateInSampleSize(options, 80, 80);
