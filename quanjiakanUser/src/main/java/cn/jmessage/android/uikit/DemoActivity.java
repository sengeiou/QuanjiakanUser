...
public class DemoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //关于注册在ReadMe中也有提到
        mMyPassword = "12345678";
        mTargetId = "hz001";//user008
        mGroupId = 10202615;
...
//            mMyName = "uikit_demo_" + username;
                    ...
                    if (JMessageClient.getMyInfo() != null) {
                        ...
                        JMessageClient.login(mMyName, mMyPassword, new BasicCallback() {
                            ...
                            public void gotResult(int status, String desc) {
                                ...
                                if (status == 0) {
                                    intent.putExtra(TARGET_ID, mTargetId);
                                    intent.putExtra(TARGET_APP_KEY,info.metaData.getString("JPUSH_APPKEY"));
                                    intent.setClass(mContext, ChatActivity.class);
                                    startActivity(intent);
                                } else {
                                    HandleResponseCode.onHandle(mContext, status, false);
    ...
    public void login() {
        ...
        loadingDialog.show();
        if(mMyName==null || mMyName.length()<1){
            mMyName = "hz001";
        }

        if(mMyPassword==null || mMyPassword.length()<1){
            mMyPassword = "12345678";
        }
        JMessageClient.login(mMyName, mMyPassword, new BasicCallback() {
            @Override
