...
public class AlbumListActivity_Single extends BaseActivity implements AdapterView.OnItemClickListener{
    ...
    public static final String PARAM_IS_SINGLE = "param_is_single";
    ...
    protected void onCreate(Bundle savedInstanceState) {
         ...
         * 是否单选--默认多选
         */
        isSinglePic = getIntent().getBooleanExtra(PARAM_IS_SINGLE,false);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        intent.setClass(AlbumListActivity_Single.this, PickPictureActivity.class);
        intent.putExtra("albumName", list.get(position).getFolderName());
        intent.putStringArrayListExtra("data", (ArrayList<String>) childList);
        intent.putExtra(PARAM_IS_SINGLE,true);
        startActivityForResult(intent, REQUEST_CODE_SELECT_PICTURE);
    }

    private static class MyHandler extends Handler {
        private final WeakReference<AlbumListActivity_Single> mActivity;

        public MyHandler(AlbumListActivity_Single activity) {
            mActivity = new WeakReference<AlbumListActivity_Single>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            AlbumListActivity_Single activity = mActivity.get();
            if (activity != null) {
                switch (msg.what) {
                    case SCAN_OK:
