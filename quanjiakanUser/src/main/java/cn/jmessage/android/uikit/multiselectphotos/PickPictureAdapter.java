...
public class PickPictureAdapter extends BaseAdapter {
    ...
    public void setmIsSingle(boolean single){
        this.mIsSingle = single;
    }
                ...
                if(mIsSingle){
                    if (!mChecked) {
                        if (mSelectMap.size() < 1) {
                            ...
                            mChecked = false;
                            Toast.makeText(mContext, mContext.getString(R.string.picture_num_limit_toast1),
                                    Toast.LENGTH_SHORT).show();
                            viewHolder.mCheckBox.setChecked(mSelectMap.get(position));
                        }
                    }else if (mSelectMap.size() <= 1) {
                    ...
                    if (mSelectMap.size() > 0) {
                        ...
                        mSendBtn.setText(mContext.getString(R.string.send) + "(" + mSelectMap.size() + "/" + "1)");
                    } else {
