...
public class BrowserViewPagerActivity extends BaseActivity implements OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public static final String PARAMS_ONLY_SHOW = "only_show";
    ...
    public static final String PARAMS_IS_ONLY_SHOW = "only_show";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isSinglePick = intent.getBooleanExtra(AlbumListActivity.PARAM_IS_SINGLE,false);
        mPosition = intent.getIntExtra("position", 0);
        onlyShow = intent.getBooleanExtra(PARAMS_IS_ONLY_SHOW,false);
        mBrowserView.initModule(onlyShow);
...
//        ArrayList<Integer> pathArray = intent.getIntegerArrayListExtra("pathArray");
        //初始化选中了多少张图片
...
//    }
    ...
    private void showSelectedNum() {
        if(isSinglePick){
            if (mSelectMap.size() > 0) {
                String sendText = mContext.getString(R.string.send) + "(" + mSelectMap.size() + "/" + "1)";
...
//                        Toast.makeText(mContext, mContext.getString(R.string.picture_num_limit_toast1), Toast.LENGTH_SHORT).show();
                        mBrowserView.setPictureSelected(mSelectMap.get(currentItem));
                    } else {
                        mSelectMap.delete(currentItem);
                    }
